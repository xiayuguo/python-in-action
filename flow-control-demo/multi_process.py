...
class Consumer(Process):
    def __init__(self, name='', queue=None, speed=LiftConfig.SPEED, volume=LiftConfig.VOLUME,
                 default_floor=LiftConfig.DEFAULT_FLOOR, floor_range=LiftConfig.FLOOR_RANGE,
                 inout_time=LiftConfig.INOUT_TIME, *args, **kwargs):
        Process.__init__(self, name=name, *args, **kwargs)
        self.name = name
        self.queue = queue
        self.speed = speed
        self.volume = volume
...
class Producer(Process):
    def __init__(self, name, queue, nums=10, *args, **kwargs):
